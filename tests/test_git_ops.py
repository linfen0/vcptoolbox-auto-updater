...
def test_check_update_needed_no_update():
    op = GitOperator("/tmp/repo", "origin", "main")
...
def test_check_update_needed_with_update():
    op = GitOperator("/tmp/repo", "origin", "main")

    def side_effect(ref):
        return "abc1234" if ref == "main" else "def5678"

    with patch.object(op, "get_commit_hash", side_effect=side_effect):
        with patch.object(op, "_run", return_value=MagicMock(stdout="3")):
            result = op.check_update_needed()
...
def test_pull_and_resolve_conflicts_with_update_no_local_changes():
    ...
    with patch.object(op, "fetch") as mock_fetch, \
         patch.object(op, "check_update_needed", return_value=mock_result), \
         patch.object(op, "_run", return_value=MagicMock(stdout="")) as mock_run, \
        ...
        mock_run.assert_any_call(["reset", "--hard", "origin/main"])


def test_pull_and_resolve_conflicts_with_local_changes():
    op = GitOperator("/tmp/repo", "origin", "main")
    mock_result = MagicMock()
    mock_result.updated = True
    mock_result.local_commit = "abc1234"
    ...
    with patch.object(op, "fetch") as mock_fetch, \
        ...
        assert result.updated
        mock_run.assert_any_call(["status", "--porcelain"], check=False)
        mock_run.assert_any_call(["stash", "push", "-m", "auto-updater-stash"], check=False)
        mock_run.assert_any_call(["reset", "--hard", "origin/main"])
